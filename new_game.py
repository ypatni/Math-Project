...
def more_text(surf, text, size, x, y):
    font = pygame.font.Font(font_name, size)
...
class Player(pygame.sprite.Sprite):
    def __init__(subject):
        ...
        subject.radius = int (subject.rect.width/1.75)
...
class Mob(pygame.sprite.Sprite):
    def __init__(subject):
        ...
        subject.rect = subject.image.get_rect()
        subject.radius = int (subject.rect.height/2)
...
def dead_screen():
    ...
    more_text(screen, "YOU DIED", 64, width / 2, height / 4)
    pygame.display.flip()
    time.sleep(5)
    show_screen()
...
dead_sound = pygame.mixer.Sound('mario.wav')
